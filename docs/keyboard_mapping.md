...
Otto reads optional `[modifier_remap]` and `[key_remap]` tables from `sc_config.toml`.  
...
Because this logic runs inside the compositor, remaps apply to all Wayland and Xwayland clients connected to Otto, but other compositors or TTYs are unaffected. For global system-wide remaps you should still use tools such as keyd or libinput hwdb overrides.
...
3. Restart the compositor after saving the file. On startup Otto will log warnings for malformed entries and continue with the valid ones.

With these tables in place, standard shortcuts—as well as compositor-level bindings—behave as if the hardware had been rewired, while remaining scoped to Otto sessions.

**Limitations to keep in mind**

- Remap tables are read once at compositor startup. You must restart Otto after editing `sc_config.toml`.
