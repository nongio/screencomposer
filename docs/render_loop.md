...
- **Calloop dispatch** – `Otto` owns a `calloop::LoopHandle`. Whenever surface state changes (e.g. via `CompositorHandler::commit`) we queue an idle source so that the next event-loop iteration is guaranteed to run, even if the backend didn’t schedule a timer itself.
...
- The compositor’s `commit` handler now calls `Otto::schedule_event_loop_dispatch()`. This schedules a zero-op idle task on the calloop, ensuring that any backend waiting with a long timeout will still wake promptly to process new buffers. This is especially useful for DRM where presentation-driven timers might otherwise delay reacting to surface updates.
