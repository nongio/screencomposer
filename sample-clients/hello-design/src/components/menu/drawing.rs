...
fn draw_menu_item(
    ...
    match item {
        MenuItem::Separator => {
            ...
            let mut separator_paint = Paint::new(Color4f::new(sep[0], sep[1], sep[2], sep[3]), None);
        ...
        MenuItem::Action { label, shortcut, enabled, .. } => {
            ...
            if is_hovered && *enabled {
                ...
                let highlight_paint = Paint::new(
                    Color4f::new(hover[0], hover[1], hover[2], hover[3]), 
                    None
                );
            ...
            let mut shortcut_paint = Paint::new(
                Color4f::new(shortcut_color[0], shortcut_color[1], shortcut_color[2], shortcut_color[3]),
                None
            );
            shortcut_paint.set_anti_alias(true);
            
            // Draw menu item label
            canvas.draw_str(label, (style.padding_left, baseline_y), menu_font, &text_paint);
            
            // Draw shortcut text
            if let Some(shortcut_text) = shortcut {
                // Measure shortcut text to right-align it
                let (shortcut_width, _) = shortcut_font.measure_str(shortcut_text, Some(&shortcut_paint));
                let shortcut_x = width - style.padding_right - shortcut_width;
                canvas.draw_str(shortcut_text, (shortcut_x, baseline_y), shortcut_font, &shortcut_paint);
            }
