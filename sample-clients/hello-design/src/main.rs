...
use components::menu::{sc_layer_shell_v1, sc_layer_v1, Menu, MenuItem, MenuItemId, Position};
...
fn get_menu_items() -> Vec<MenuItem> {
    vec![
        ...
        MenuItem::action("file.new_window", "New Window")
            .shortcut("⌘N")
            .build(),
        MenuItem::action("file.new_incognito", "New Incognito Window")
            .shortcut("⇧⌘N")
            .build(),
        MenuItem::separator(),
        ...
        MenuItem::submenu("export", "Export")
            ...
            .build(),
        MenuItem::separator(),
        MenuItem::action("file.close_tab", "Close Tab")
            .shortcut("⌘W")
            .build(),
        MenuItem::action("file.print", "Print...")
            .shortcut("⌘P")
            .build(),
    ]
}
...
impl WindowHandler for AppData {
    ...
    fn configure(
        ...
        if !self.configured {
            ...
            println!(
                "Created Skia context and surface: {}x{} (buffer scale {})",
                width, height, buffer_scale
            );
                ...
                if let Some(ref mut main_surface) = self.main_surface {
                    main_surface.draw(ctx, |canvas| {
                        ...
                        let typeface = font_mgr
                            ...
                            .unwrap_or_else(|| {
                                font_mgr.legacy_make_typeface(None, font_style).unwrap()
                            });
    ...
    fn event(
        ...
        match event {
            wl_pointer::Event::Enter {
                ...
                // Set cursor
                if let (Some(cursor_theme), Some(cursor_surface)) =
                    (&mut state.cursor_theme, &state.cursor_surface)
                {
            ...
            wl_pointer::Event::Motion {
                ...
                if let Some(ref current_surface) = state.current_surface {
                    if state.menu.owns_surface(current_surface) {
                        state
                            .menu
                            .on_pointer_motion(current_surface, surface_x, surface_y, qh);
                    }
                }
                ...
                if let Some((parent_path, submenu_idx)) = state.menu.should_open_submenu() {
                    println!(
                        "Opening submenu at index {} (parent path: {:?})",
                        submenu_idx, parent_path
                    );
