...
use hello_design::{
    components::menu::{
        sc_layer_shell_v1, sc_layer_v1, Anchor, Gravity, MenuItem, MenuItemId, Position,
    },
    components::menu_bar::{surface::MenuBarSurface, MenuBar},
...
use wayland_client::{
    ...
    protocol::{
        wl_keyboard, wl_output, wl_pointer, wl_seat, wl_subcompositor, wl_subsurface, wl_surface,
    },
        ...
        if let Some(ref mut ctx) = self.skia_context {
            if let Some(ref mut main_surface) = self.main_surface {
                main_surface.draw(ctx, |canvas| {
                    ...
                    let paint =
                        skia_safe::Paint::new(skia_safe::Color4f::new(0.2, 0.2, 0.2, 1.0), None);
    ...
    fn configure(
        ...
        if !self.configured {
            ...
            menu_bar.add_item(
                "View",
                vec![
                    MenuItem::action("view.fullscreen", "Toggle Fullscreen")
                        .shortcut("F11")
                        .build(),
                    MenuItem::separator(),
                    MenuItem::action("view.zoom_in", "Zoom In")
                        .shortcut("⌘+")
                        .build(),
                    MenuItem::action("view.zoom_out", "Zoom Out")
                        .shortcut("⌘-")
                        .build(),
                ],
            );
            ...
            Event::Enter {
                ...
                if let Some(ref mut menu_bar_surface) = app_data.menu_bar_surface {
                    if let Some(active_label) = menu_bar_surface.menu_bar().active_menu() {
                        let active_label = active_label.to_string();
                        if let Some(menu) =
                            menu_bar_surface.menu_bar_mut().get_menu_mut(&active_label)
                        {
            ...
            Event::Motion {
                ...
                if let Some(ref mut menu_bar_surface) = app_data.menu_bar_surface {
                    if let Some(active_label) = menu_bar_surface.menu_bar().active_menu() {
                        ...
                        if let Some(menu) = menu_bar_surface
                            .menu_bar_mut()
                            .get_menu_mut(&active_label_clone)
                        {
                            // Check if current surface belongs to this menu (root or any submenu)
                            let is_menu_surface = app_data
                                .current_surface
                                .as_ref()
                            ...
                            if is_menu_surface {
                                if let Some(current_surf) = app_data.current_surface.as_ref() {
                                    ...
                                    if let Some((parent_path, item_index)) =
                                        menu.should_open_submenu()
                                    {
                ...
                if on_menubar {
                    if let Some(ref mut menu_bar_surface) = app_data.menu_bar_surface {
                        if let Some((label, x_pos, changed)) = menu_bar_surface.handle_hover(x, y) {
                            ...
                            if changed {
                                if let (Some(ref window), Some(display_ptr)) =
                                    (&app_data.window, app_data.display_ptr)
                                {
                                    ...
                                    for menu_label in &all_labels {
                                        if let Some(menu) =
                                            menu_bar_surface.menu_bar_mut().get_menu_mut(menu_label)
                                        {
                                            if menu.is_visible() {
                                                menu.hide();
                                            }
                                    ...
                                    if let Some(menu) =
                                        menu_bar_surface.menu_bar_mut().get_menu_mut(&label)
                                    {
                                        let position = hello_design::components::menu::Position {
                                            ...
                                            gravity:
                                                hello_design::components::menu::Gravity::BottomRight,
                                        };
                        ...
                        if let Some((label, x_pos)) = menu_bar_surface.handle_click(x, y) {
                            ...
                            for other_label in &all_labels {
                                if other_label != &label {
                                    if let Some(menu) =
                                        menu_bar_surface.menu_bar_mut().get_menu_mut(other_label)
                                    {
                                        menu.hide();
                                    }
                                }
                            ...
                            if is_active {
                                if let (Some(window), Some(display_ptr)) =
                                    (&app_data.window, app_data.display_ptr)
                                {
                                    if let Some(menu) =
                                        menu_bar_surface.menu_bar_mut().get_menu_mut(&label)
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if sc_layer_shell_v1.is_none() {
        eprintln!(
            "Warning: sc_layer_shell_v1 not available, menus will not have blur/shadow effects"
        );
    }
