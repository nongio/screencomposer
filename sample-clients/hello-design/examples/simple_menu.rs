use hello_design::{
    components::{
        menu::{Anchor, Gravity, Menu, MenuItem, MenuItemId, Position, sc_layer_shell_v1, sc_layer_v1}, window::SimpleWindow
...
use smithay_client_toolkit::{
    ...
    shell::{
        xdg::{
            popup::{PopupConfigure, PopupHandler, PopupData},
        ...
        if let Some(ref mut ctx) = self.skia_context {
            if let Some(ref mut main_surface) = self.main_surface {
                main_surface.draw(ctx, |canvas| {
                    ...
                    let typeface = font_mgr.match_family_style("Inter", skia_safe::FontStyle::normal())
                    ...
                    let status_blob = TextBlob::new(status_text, &font).expect("Failed to create text blob");
        ...
        for event in events {
            match &event.kind {
                PointerEventKind::Enter { .. } => {
                    self.menu.on_pointer_enter(&event.surface, event.position.0, event.position.1);
                }
                ...
                PointerEventKind::Motion { .. } => {
                    ...
                    self.menu.on_pointer_motion(&event.surface, event.position.0, event.position.1, qh);
                    ...
                    if *button == 273 && !self.menu.is_visible() {
                        if let (Some(window), Some(display_ptr)) = (&self.window, self.display_ptr) {
                            ...
                            match self.menu.open_menu(
                                ...
                                Ok(_) => {
                                    println!("Menu opened at ({}, {})", self.pointer_x, self.pointer_y);
                                    self.render_window();
                                }
                        ...
                        self.menu.on_pointer_button(*button, wl_pointer::ButtonState::Pressed);
                        self.render_window();
                    }
                }
                PointerEventKind::Release { button, .. } => {
                    self.menu.on_pointer_button(*button, wl_pointer::ButtonState::Released);
                }
...
impl SeatHandler for AppData {
    ...
    fn new_capability(&mut self, _conn: &Connection, _qh: &QueueHandle<Self>, seat: wl_seat::WlSeat, capability: Capability) {
        if capability == Capability::Pointer && self.pointer.is_none() {
            let pointer = self.seat_state.get_pointer(_qh, &seat).expect("Failed to create pointer");
