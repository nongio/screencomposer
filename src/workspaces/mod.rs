...
pub use workspace::Application;
pub use workspace::Window;
...
impl Workspaces {
    ...
    pub fn add_workspace(&mut self) {
        self.with_model_mut(|m| {
            ...
            // workspace.workspace_layer.set_position((), None);
        ...
        self.with_model_mut(|m| {
            ...
            if n < m.workspaces.len() {
                ...
                if m.current_workspace >= m.workspaces.len() {
                    ...
                    self.scroll_to_workspace(m.current_workspace);
                }
                self.notify_observers(&m);
            }
        });
    }
    pub fn get_current_workspace(&self) -> Arc<Workspace> {
    ...
    pub fn get_or_add_window_view(
        ...
        let insert = window_views.entry(object_id.clone()).or_insert_with(|| {
            WindowView::new(self.layers_engine.clone(), parent_layer_id, window)
        });
        insert.clone()
    }
            ...
            if let Some(desktop_entry) = desktop_entry {
                if let Some(mut model_mut) = acquire_write_lock_with_retry(&model) {
                    ...
                    if app.icon_path != icon_path {
                        ...
                        instance.notify_observers(&model_mut.clone());
                    }
                }
            }
        });
    ...
    pub(crate) fn update_window(&self, id: &ObjectId, model: &WindowViewBaseModel) {
        ...
        if let Some(window) = workspace_model.windows_cache.get(id) {
            ...
            workspace_model
                .windows_cache
                .insert(id.clone(), window.clone());
        }
    }

    // updates the workspace model using elemenets from Space
    pub(crate) fn update_with_window_elements(
        &self,
        windows: Vec<(WindowElement, lay_rs::prelude::Layer, WindowViewBaseModel)>,
    )
    // where
    // I: Iterator<Item = (WindowElement, lay_rs::prelude::Layer, WindowViewBaseModel)>,
    {
        {
    ...
    pub fn set_current_workspace(&self, i: usize) {
        self.with_model_mut(|m| {
            if i > m.workspaces.len() - 1 {
                return;
            }
            m.current_workspace = i;
            self.scroll_to_workspace(i);

            self.notify_observers(&m);
        });
    }
    fn scroll_to_workspace(&self, i: usize) {
        let width = self.workspaces_layer.render_size().x;
        self.workspaces_layer.set_position(
            (-(width + 100.0) * i as f32, 0.0),
            Some(Transition {
                delay: 0.0,
                timing: TimingFunction::Spring(Spring::new_with_velocity(1.0, 90.0, 30.0, 0.0)),
            }),
        );
    }
}
